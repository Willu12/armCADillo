...
public:
  ...
  void flipY() {
    ...
    const int rowSize = width_ * 4; // 4 bytes per pixel (RGBA)

    for (uint32_t y = 0; y < height_; ++y) {
      // Source row from bottom
...
public:
  ...
  createIntersectionTextures(
    ...
    auto texture1 =
        std::make_shared<IntersectionTexture>(points1, bounds.first);
    auto texture2 =
        std::make_shared<IntersectionTexture>(points2, bounds.second);
    return {std::move(texture1), std::move(texture2)};
  }

  void bind() const { texture_->bind(0); }
...
private:
  ...
  void drawLine(const std::vector<algebra::Vec2f> &surfacePoints) {
    ...
    //  const algebra::Vec2f size = max - min;
  ...
  std::vector<std::pair<uint32_t, uint32_t>> findInteriorSamplePoints() {
    ...
    std::vector<std::vector<bool>> isInterior(kHeight,
                                              std::vector<bool>(kWidth, false));
    std::vector<std::vector<bool>> isBackground(
        kHeight, std::vector<bool>(kWidth, false));
